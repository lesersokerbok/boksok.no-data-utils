...
function cleanContent(content) {
    ...
    // remove <div><br />..</div>
    content = content.replace(/<div>\s*?<br\/>\s*?<\/div>/, '<br />');
    
    // <br/> -> <br />
    content = content.replace(/<br\/>/, '<br />')
     
    // <div>(...)</div> -> <p>(...)</p>
    content = content.replace(/<div>(.*?)<\/div>/igm, '<p>$1</p>');
...
function tagsArrayFromItemCategory(itemCat, bookID, lsbCatKey, textCategories) {
    ...
    //book.lsb_tax_customization = customizationTaxFromMenuLink(book.id, menuItems, menuLinks);
    function customizationTaxFromMenuLink(bookId, menuItems, menuLinks) {
      var filterIds = [11];
      return tagsArrayFromMenuLink(bookId, menuItems, menuLinks, filterIds);
    }
    //book.lsb_tax_audience = audienceTaxFromMenuLink(book.id, menuItems, menuLinks);
    function audienceTaxFromMenuLink(bookId, menuItems, menuLinks) {
      var filterIds = [12, 20, 21];
      return tagsArrayFromMenuLink(bookId, menuItems, menuLinks, filterIds);
    }
    
    //book.lsb_tax_lsb_cat = lsbCatTaxFromMenuLink(book.id, menuItems, menuLinks);
    function lsbCatTaxFromMenuLink(bookId, menuItems, menuLinks) {
      var filterIds = [3, 4, 5, 6, 7, 8, 18, 19];
      var tags = tagsArrayFromMenuLink(bookId, menuItems, menuLinks, filterIds);
      
      tags = _.uniq(tags);
      
      if(_.contains(tags, 'bliss') && _.contains(tags, 'piktogram')) {
        tags.push('bliss & piktogram');
      }
      
      if(_.contains(tags, 'tegnspråk') && _.contains(tags, 'nmt') ) {
        tags.push('Tegnskpråk & NMT');
      }
      
      if(_.contains(tags, 'punktskrift') && _.contains(tags, 'følebilder')) {
        tags.push('Punktskrift & følebilder');
      }
      
      return tags;
    }
    
    function genreTaxFromMenuLink(bookId, menuItems, menuLinks) {
      var filterIds = [9, 10];
      return tagsArrayFromMenuLink(bookId, menuItems, menuLinks, filterIds);
    }
    
    function tagsArrayFromMenuLink(bookID, menuItems, menuLinks, filterIds) {
...
function convertItemToJSON(books, item, textCategories, menuLinks, menuItems, contentItems) {
    ...
    // Sjanger
    book.lsb_genre = [];
    book.lsb_genre.concat(
      tagsArrayFromItemCategory(item['CUSTOM_CAT_3'], book.id, 'LSB_GENRE', textCategories)
    );
    book.lsb_genre.concat(
      genreTaxFromMenuLink(book.id, menuItems, menuLinks)
    );
    ...
    // Mange bilder
    // Lydbok
    book.lsb_tax_customization = [];
    book.lsb_tax_customization.concat(
      tagsArrayFromItemCategory(item['CUSTOM_CAT_2'], book.id, 'LSB_EXTRA', textCategories)
    );
    book.lsb_tax_customization.concat(
      customizationTaxFromMenuLink(book.id, menuItems, menuLinks)
    );
